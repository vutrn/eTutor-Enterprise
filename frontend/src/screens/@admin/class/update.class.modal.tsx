...
const UpdateModal = ({ modalVisible, setModalVisible }: IProps) => {
  ...
  // Set form values when selected class changes
  useEffect(() => {
    if (selectedClass) {
      ...
      <ModalContent className="max-h-[85vh]">
        ...
        <ModalFooter>
          ...
          <Button
            ...
            isDisabled={isSubmitting || loadingClassUpdate}
            action="primary"
          >
            {isSubmitting || loadingClassUpdate ? (
              <ActivityIndicator size="small" color="#ffffff" />
            ) : (
              <ButtonText>Update Class</ButtonText>
            )}
          </Button>
        </ModalFooter>
      </ModalContent>
